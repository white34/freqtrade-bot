# ============================================================
#  DeadZoneDip — inverted-gate experiment
#
#  *** VERDICT 2026-06-12: DEAD — DO NOT DEPLOY ***
#  Identical logic to MeanRevLong, gate INVERTED (trades only when
#  BTC regime is OFF). Full: -78.71% / WR 46.9%. 2022: -48.98% /
#  Sharpe -3.54. OOS: -45.35%. The same setup that makes +715%
#  gated-on loses -79% gated-off — definitive proof of the gate.
#  Even relative-strength alts (above own EMA200) sink with BTC.
#  See RESEARCH_LOG.md. (Header below is inherited; class renamed.)
#
...
#
#  Archived benchmarks (5y 2021â†’2026-05, 29-30 pairs, 5x):
#    Full: +237.98% / Sharpe 1.21 / Calmar 8.43 / DD 28.01% / WR 63.5% / 1529 trades
#    OOS 22mo (2024-07â†’2026-05): +109.82% / Sharpe 1.82 / DD 17.41% / WR 69%
#
#  RISK MODEL at 5x leverage (METHODOLOGY.md Â§1 â€” ratios are
#  LEVERAGED stake ratios, NOT price moves):
#    stoploss -0.07  = -1.4% price = -7% of stake â‰ˆ -0.9% of wallet
#                      (stake â‰ˆ 12.5% of capital at 8 slots)
#    ROI 0.20/0.10/0.05/0.024/0.01
#                    = +4.0% / +2.0% / +1.0% / +0.48% / +0.2% price
#    trailing: arms at 0.18 (=3.6% price), trails 0.122 (=2.44% price)
#  This is a tight-stop scalper by design â€” that IS the validated
#  system. Do not "widen the stop to -35%" expecting old behavior.
#
#  Current benchmarks (2026-06-12, doubled-ROI ladder, data 2021â†’2026-06):
#    Full: +448.87% / CAGR 36.8% / DD 18.6% / WR 56.8% / 1651 trades
#    OOS 2024-07â†’: +145.67% / Sharpe 2.71 / PF 1.41 / DD 18.6%
# ============================================================
...
class DeadZoneDip(IStrategy):
    ...
    # worse (+148.7%) â€” winners round-trip. See RESEARCH_LOG 2026-06-12.
    minimal_roi = {
        "0":   0.20,
        "60":  0.10,
        "180": 0.05,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # INVERTED GATE: dead zones only — relative-strength dips while BTC is weak
        regime_ok = (dataframe["btc_regime_up_4h"] == 0)
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        if int(last.get("btc_regime_up_4h", 1)) != 0:
            return None
        try:
